...
router.post('/send-resend-email', authenticate, authorize('recruiter', 'admin'), async (req, res) => {
  try {
    const { applicationId, subject, body, templateId, email: providedEmail } = req.body as { applicationId: string; subject?: string; body?: string; templateId?: string; email?: string };
    console.log('[Resend] Incoming email request:', { applicationId, subject, body, templateId, providedEmail });
    ...
    try {
      ...
      return res.status(500).json({ message: 'Resend email error', error: emailErr instanceof Error ? emailErr.message : emailErr });
    }
  } catch (err: any) {
