...
router.post('/jobs/:id/apply', upload.single('resume'), async (
  ...
  try {
    ...
    if (!firstName || !lastName || !email) return res.status(400).json({ message: 'Missing name or email fields' });
...
router.delete('/applications/:id', authenticate, authorize('recruiter', 'admin'), async (req, res) => {
  try {
...
router.patch('/applications/:id/status', authenticate, authorize('recruiter', 'admin'), async (req, res) => {
  try {
    const id = req.params.id;
    const { status } = req.body as { status?: string };
    const allowed = ['new', 'shortlisted', 'rejected', 'interview'];
...
router.post('/applications/:id/message', authenticate, authorize('recruiter', 'admin'), async (req, res) => {
  try {
    const id = req.params.id;
    const { subject, body, templateId, email: providedEmail } = req.body as { subject?: string; body?: string; templateId?: string; email?: string };
    // if caller provided an email override, validate format
    if (providedEmail) {
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(providedEmail)) return res.status(400).json({ message: 'Invalid email format' });
    }
