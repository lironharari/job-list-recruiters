...
export const markdownToHtml = (md: string) => {
  ...
  for (let raw of lines) {
    ...
    if (h) { if (inList) { out += '</ul>'; inList = false; } const lvl = h[1].length; out += `<h${lvl}>${escapeHtml(h[2])}</h${lvl}>`; continue; }
    const li = line.match(/^[-*+]\s+(.*)$/);
    if (li) { if (!inList) { out += '<ul>'; inList = true; } out += `<li>${escapeHtml(li[1])}</li>`; continue; }
